...
class ChipsController extends AppController
{
    ...
    public $uses = array('Excel', 'Chip', 'Chipstmp');
    public $layout = 'viva';
        ...
        if ($this->Excel->save($this->data['Excelg']))
        {
            ...
            foreach ($rowIterator as $row)
            {
                ...
                foreach ($cellIterator as $cell)
                {
                    ...
                    {
                        $fechaEntregaSD = $cell->getCalculatedValue();
                        $time = PHPExcel_Shared_Date::ExcelToPHP($fechaEntregaSD);
                        $fechaEntregaSDPhp = date('Y-m-d', $time);
                        $array_data[$rowIndex][$cell->getColumn()] = $fechaEntregaSDPhp;
                    } elseif ('F' == $cell->getColumn())
                    {
                        $array_data[$rowIndex][$cell->getColumn()] = $cell->getCalculatedValue();
                    } elseif ('G' == $cell->getColumn())
            ...
            foreach ($array_data as $d)
            {
                ...
                $this->request->data[$i]['Chip']['cliente'] = $d['E'];
                $this->request->data[$i]['Chip']['fv'] = $d['F'];
                $fechaExcelMysql = str_replace("\'", "", $d['G']);
                $this->request->data[$i]['Chip']['fecha'] = $fechaExcelMysql;
                $fechaEntregaSDMysql = str_replace("\'", "", $d['H']);
                $this->request->data[$i]['Chip']['fecha_entrega_subdealer'] = $fechaEntregaSDMysql;
                $this->request->data[$i]['Chip']['codigo_subdealer'] = $d['I'];
                $this->request->data[$i]['Chip']['nombre_subdealer'] = $d['J'];
                $this->request->data[$i]['Chip']['codigo_mercado'] = $d['K'];
                $this->request->data[$i]['Chip']['nombre_distribuidor'] = $d['L'];
                $this->request->data[$i]['Chip']['nombre_distribuidor'] = $d['M'];
