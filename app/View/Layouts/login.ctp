...
<script>
		...
		$(document).ready(function()
		{
			 ...
			 */
			$('#form-login222').submit(function(event)
			{
				// Values
				...
				{
					// Display message
					displayError('Por favor coloque su usuario');
					return false;
				}
				else if (pass.length === 0)
				{
					// Remove empty login message if displayed
					formBlock.clearMessages('Por favor coloque su usuario');

					// Display message
					displayError('Por favor coloque su passwrord');
					return false;
				}
				else
				{
					...
					// Show progress
					displayLoading('Verficando datos...');
          ...
          $('#form-login').submit();
					...
					//setTimeout(function() {
						//document.location.href = "<?php //echo $this->Html->url(array('controller'=>'Users', 'action'=>'login')); ?>/"+login+"/"+pass,
            //$('#form-login').submit();
					//}, 2000);
				}
			});
